...
import { Save, Edit2, Trash2, PackageCheck } from 'lucide-react'
...
const PrecoEntrega = () => {
  ...
  const [precos, setPrecos] = useState<PrecoEntregaType[]>([])
  ...
  const fetchPrecos = async () => {
    ...
    if (error) {
      ...
      toast.error('Erro ao carregar preços por Entrega')
    } else {
      setPrecos(data || [])
    }
    setFetching(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        toast.success(`Preço atualizado para ${formatCurrency(parseFloat(valor))}/entrega`, 'Preço Atualizado')
      } else {
        ...
        toast.success(`Novo preço cadastrado: ${formatCurrency(parseFloat(valor))}/entrega`, 'Preço Criado')
      }
  ...
  const handleEdit = (preco: PrecoEntregaType) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirm(`Tem certeza que deseja excluir o preço de ${preco ? formatCurrency(preco.valor) : '?'}/entrega?`)) return
    if (!user) return
  ...
  const toggleAtivo = async (preco: PrecoEntregaType) => {
    if (!user) return
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Valor por Entrega</h1>
      ...
      <div className="card mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="label">Valor por Entrega (R$)</label>
            <input
              type="number"
              step="0.01"
              value={valor}
      ...
      <div className="card">
        ...
        {fetching ? (
          ...
          <EmptyState
            ...
            description="Configure o valor fixo por entrega. Este valor será adicionado automaticamente ao total de cada lançamento."
          />
        ) : (
